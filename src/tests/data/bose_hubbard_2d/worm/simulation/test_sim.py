...
class TestWormSimulation(WormOutputTests, WormSimulationTests):
    ...
    def test_init_for_non_existant_save_dir(
        ...
        test_simulation = WormSimulation(
            input_parameters=input_parameters, save_dir=non_existant_save_dir
        )
    ...
    def test_get_tune_dir_path(test_simulation: WormSimulation) -> None:
        assert (
            test_simulation.get_tune_dir_path(test_simulation.save_dir)
    ...
    def fixture_test_checkpoint(
        test_simulation: WormSimulation, test_checkpoint_status: str
    ) -> Iterator[None]:
    ...
    def test_set_get_extension_sweeps_in_checkpoints(
        ...
        for extension_sweeps in (10, 20, 30):
            if test_checkpoint_status in [
                ...
                assert (
                    test_simulation.get_extension_sweeps_from_checkpoints()
                    == extension_sweeps
                )
    ...
    @staticmethod
    def test_output(
        test_simulation: WormSimulation, sim_output_valid_densities: Iterator[None]
    ) -> None:
    ...
    def test_observables(
        ...
        for obs_type, obs_names in test_simulation.observables.observable_names.items():
            for obs_name in obs_names:
                assert (
                    test_simulation.observables.get_expectation_value(
                        obs_type, obs_name
                    )
                    is not None
                )
    ...
    def test_plotting(
        ...
        for obs_name in itertools.chain.from_iterable(
            ...
            produced_files = (
                test_simulation.get_plot_dir_path(test_simulation.save_dir) / obs_name
            ).glob("*.png")
            assert any(produced_files)

        test_simulation.plot_inputs()
        assert (
            test_simulation.get_plot_dir_path(test_simulation.save_dir) / "inputs.png"
        ).exists()

        test_simulation.plot_phase_diagram_inputs()
        assert (
            test_simulation.get_plot_dir_path(test_simulation.save_dir)
            / "phase_diagram_inputs.png"
        ).exists()

    @staticmethod
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "test_dispatcher_return_code",
        [ReturnCode.SUCCESS, ReturnCode.FAILURE],
        indirect=True,
    )
    @pytest.mark.parametrize(
        "test_dispatcher_expected_input_file_type", [".ini", ".h5"], indirect=True
    )
