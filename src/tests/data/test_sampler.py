...
class SamplerCases:
    ...
    def case_batchsizes(
        dataset: FakeDataset, batch_size: int, n_duplicates: int
    ) -> tuple[MDuplicatesPerBatchSampler, list[int]]:
        """Return a sampler with the allowed batch sizes."""

        allowed_batch_sizes = [batch_size] + (
            [(len(dataset) * n_duplicates) % batch_size]
            if (len(dataset) * n_duplicates) % batch_size != 0
            else []
        )
        sampler = MDuplicatesPerBatchSampler(
            dataset,
    ...
    def case_batchsizes_drop_last(
        dataset: Dataset, batch_size: int, n_duplicates: int
    ) -> tuple[MDuplicatesPerBatchSampler, list[int]]:
    ...
    def case_complete(
        dataset: FakeDataset, batch_size: int, n_duplicates: int, shuffle: bool
    ) -> tuple[MDuplicatesPerBatchSampler, FakeDataset]:
    ...
    @parametrize_with_cases("shuffle", cases=ShuffleCases)
    def case_shuffled(
        dataset: FakeDataset, shuffle: bool
    ) -> tuple[MDuplicatesPerBatchSampler, FakeDataset, bool]:
        """Return a sampler that returns shuffled indices."""
        return (
            MDuplicatesPerBatchSampler(
                dataset, n_duplicates=1, batch_size=1, shuffle=shuffle, drop_last=False
            ),
            dataset,
            shuffle,
        )
    ...
    def case_duplicated(
        dataset: FakeDataset, n_duplicates: int, shuffle: bool, drop_last: bool
    ) -> tuple[MDuplicatesPerBatchSampler, int, bool]:
    ...
    def case_length(
        ...
        shuffle: bool,
        drop_last: bool,
    ) -> tuple[MDuplicatesPerBatchSampler, int]:
...
class TestNDuplicatesPerBatchSampler:
    ...
    @parametrize_with_cases(
        "sampler, allowed_batch_sizes",
        cases=SamplerCases,
        filter=ft.has_tag("batchsize"),
    )
    def test_batchsize(
        sampler: MDuplicatesPerBatchSampler, allowed_batch_sizes: list[int]
    ) -> None:
    ...
    @staticmethod
    @parametrize_with_cases(
        "sampler, dataset", cases=SamplerCases, filter=ft.has_tag("complete")
    )
    def test_complete(
        sampler: MDuplicatesPerBatchSampler, dataset: FakeDataset
    ) -> None:
    ...
    @staticmethod
    @parametrize_with_cases(
        "sampler, dataset, shuffle", cases=SamplerCases, filter=ft.has_tag("shuffled")
    )
    def test_shuffled(
        sampler: MDuplicatesPerBatchSampler, dataset: FakeDataset, shuffle: bool
    ) -> None:
        ...
        ) == shuffle  # not impossible but very unlikely

    @staticmethod
    @parametrize_with_cases(
        "sampler, n_duplicates, drop_last",
        cases=SamplerCases,
        filter=ft.has_tag("duplicated"),
    )
    def test_duplicated(
        sampler: MDuplicatesPerBatchSampler, n_duplicates: int, drop_last: bool
    ) -> None:
        ...
        if not drop_last:
            ...
            assert all(
                count == n_duplicates for count in counts if count != min(counts)
            )

    @staticmethod
    @parametrize_with_cases(
        "sampler, expected_length", cases=SamplerCases, filter=ft.has_tag("length")
    )
