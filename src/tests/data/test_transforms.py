...
class GroupElementsCases:
    ...
    def case_group_elements_multiply(
        ...
        and the composed group element."""
        return [
            GroupElement(name="multiply_2",
                         transform=lambda x: x * 2,
                         inverse_transform=lambda x: x / 2),
            GroupElement(name="multiply_0.5",
                         transform=lambda x: x * 0.5,
                         inverse_transform=lambda x: x / 0.5),
            GroupElement(name="multiply_3",
                         transform=lambda x: x * 3,
                         inverse_transform=lambda x: x / 3),
        ], torch.Tensor([3.0]), torch.Tensor([9.0]), GroupElement(
            name="multiply_3",
            transform=lambda x: x * 3,
            inverse_transform=lambda x: x / 3)
...
class TestGroupElement:
    ...
    @staticmethod
    @parametrize_with_cases("group_elements, x, expected, composed",
                            cases=GroupElementsCases)
    def test_from_group_elements(group_elements: list[GroupElement], x: torch.Tensor,
                                 expected: torch.Tensor,
                                 composed: GroupElement) -> None:
