...
def case_random_predictions_targets_pair() -> (
    ...
    batch = MultipleSizesBatch(
        inputs=[], outputs=targets, sample_ids=[], group_elements=[], size=len(sizes)
    )
    return predictions, batch


@parametrize_with_cases(
    "predictions, batch", cases=[case_random_predictions_targets_pair]
)
def test_mse_loss_and_metric_equal(
    predictions: list[torch.Tensor], batch: MultipleSizesBatch
) -> None:
