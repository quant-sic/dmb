...
def simulate_mu_cut(muU_min: float = typer.Option(0.0, help="Minimum mu offset"),
                    muU_max: float = typer.Option(3.0, help="Maximum mu offset"),
                    muU_num_steps: int = typer.Option(10,
    ...
    target_dir = (REPO_DATA_ROOT /
                  f"bose_hubbard_2d/mu_cut/{zVU}/{ztU}/{L}/simulations")
    ...
    async def run_sample(sample_id: int) -> None:
        async with semaphore:
            await simulate(
                ...
                simulation_name="mu_cut_{}_{:.3f}_{}".format(zVU, muU_out[sample_id],
                                                             sample_id),
                L=L,
