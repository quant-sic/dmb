...
def from_potential(
    ...
    potential = torch.load(potential_file_path, map_location="cpu",
                           weights_only=True).cpu().detach().numpy()
    now = datetime.now().strftime("%Y-%m-%d-%H-%M-%S")

    U_on = 4.0 * t_hop / ztU

    asyncio.run(
        simulate(parent_dir=simulations_dir,
                 simulation_name=f"{run_name}_{now}",
                 L=potential.shape[0],
                 mu=potential,
                 t_hop_array=np.full((2, potential.shape[0], potential.shape[1]),
                                     t_hop),
                 U_on_array=np.full((potential.shape[0], potential.shape[1]), U_on),
                 V_nn_array=np.full((potential.shape[0], potential.shape[1]),
                                    zVU * U_on / 4),
                 power=1.0,
                 mu_offset=0.0))
