...
class LitModel1d(pl.LightningModule):
    ...
    def __init__(
        ...
        observable_mapping = {
            ...
            "covariance": 2,
            "r_value": 3,
            "r_value_oscillation": 3,
            "density2": 4,
            "density3": 5,
            "density_middle": 6,
            "current_correlation": 7,
            "current_correlation_excited_middle": 8,
            "density_matrix_off_diag": 9,
        }
    ...
    def training_epoch_end(
            self, training_step_outputs: list[dict[str, torch.Tensor]]) -> None:
    ...
    def set_keras_weights(self, weights):
        for (name, p), keras_weight in zip(self.named_parameters(), weights):
            if "bias" in name:
                ...
                assert (torch.from_numpy(keras_weight).permute(3, 2, 1,
                                                               0).shape == p.shape)
