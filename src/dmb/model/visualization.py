...
class AttentionVisualizer:
    ...
    def visualize_attention(
        ...
        self._plot_attention_ratios(
            input_tensor.cpu(), save_path, figsize, max_num_cols
        )
        self._plot_fourier_frequency_analysis(
            input_tensor.cpu(), save_path, figsize, max_num_cols
        )
    ...
    def _plot_attention_maps(
        self, save_path: Path, figsize: Tuple[int, int], max_num_cols: int
    ):
        ...
        for att_type in attention_types:
            ...
            maps = {
                ...
                if att_type in k and "gate" not in k
            }
            ...
            plt.tight_layout()
            plt.savefig(
                save_path / f"{att_type}_attention_maps.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _plot_attention_comparison(
        self, input_tensor: torch.Tensor, save_path: Path, figsize: Tuple[int, int]
    ):
        ...
        last_block_idx = max(
            [
                int(k.split("_")[1])
                for k in self.attention_maps.keys()
        ...
        plt.tight_layout()
        plt.savefig(
            save_path / "attention_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()

    def _plot_attention_ratios(
        self,
        input_tensor: torch.Tensor,
        save_path: Path,
        ...
        for att_type in attention_types:
            ...
            input_maps = {
                k.replace(f"_{att_type}", f"_{att_type}_input"): v
                for k, v in output_maps.items()
                if k.replace(f"_{att_type}", f"_{att_type}_input")
                in self.attention_maps
            }
            ...
            fig.suptitle(
                f"{att_type.upper()} Attention Ratios (Output/Input)", fontsize=16
            )
            ...
            for i, (output_name, output_map) in enumerate(output_maps.items()):
                ...
                # Average across channels and batch for visualization
                input_avg = input_map[0].mean(
                    dim=0
                )  # First sample, average across channels
                output_avg = output_map[0].mean(
                    dim=0
                )  # First sample, average across channels
                ...
                im = axes[i].imshow(
                    ratio_numpy, cmap="RdBu_r", aspect="auto", vmin=vmin, vmax=vmax
                )
                axes[i].set_title(
                ...
                if vmin <= 1.0 <= vmax:
                    ...
                    cbar.ax.axhline(
                        y=1.0, color="black", linestyle="--", alpha=0.7, linewidth=1
                    )
            ...
            plt.tight_layout()
            plt.savefig(
                save_path / f"{att_type}_attention_ratios.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def _plot_ratio_summary(self, save_path: Path, figsize: Tuple[int, int]):
        ...
        for att_type in attention_types:
            ...
            if output_key in self.attention_maps and input_key in self.attention_maps:
                ...
                att_input = self.attention_maps[input_key][0].mean(
                    dim=0
                )  # First sample, average across channels
                att_output = self.attention_maps[output_key][0].mean(
                    dim=0
                )  # First sample, average across channels
        ...
        fig, axes = plt.subplots(
            1, n_types + 1, figsize=(figsize[0] * (n_types + 1), figsize[1])
        )
        ...
        for i, (att_type, ratio_map) in enumerate(zip(valid_types, ratio_maps)):
            im = axes[i + 1].imshow(
                ratio_map, cmap="RdBu_r", aspect="auto", vmin=vmin, vmax=vmax
            )
            axes[i + 1].set_title(
                f"{att_type.upper()} Ratio\n(μ={ratio_map.mean():.2f}, σ={ratio_map.std():.2f})"
            )
            axes[i + 1].axis("off")
        ...
        plt.suptitle(
            f"Attention Module Ratio Summary - Block {last_block_idx}", fontsize=16
        )
        plt.tight_layout()
        plt.savefig(
            save_path / "attention_ratio_summary.png", dpi=300, bbox_inches="tight"
        )
        plt.close()

    def _plot_fourier_frequency_analysis(
        self,
        input_tensor: torch.Tensor,
        save_path: Path,
        figsize: Tuple[int, int],
        ...
        fft_input_maps = {
            k: v for k, v in self.attention_maps.items() if "fft_input" in k
        }
        fft_output_maps = {
            k: v for k, v in self.attention_maps.items() if "fft_output" in k
        }
        ...
        for i, block_name in enumerate(sorted(fft_input_maps.keys())):
            ...
            axes[row_input, col].set_title(
                f'{block_name.replace("_fft_input", "")} - Input FFT Magnitude'
            )
            ...
            im2 = axes[row_ratio, col].imshow(
                freq_ratio, cmap="RdBu_r", aspect="auto", vmin=0.5, vmax=2.0
            )  # Center around 1.0
            axes[row_ratio, col].set_title(
                f"Frequency Ratio (Output/Input)\n(μ={freq_ratio.mean():.2f})"
            )
        ...
        plt.tight_layout()
        plt.savefig(
            save_path / "fourier_frequency_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
    ...
    def _plot_frequency_spectrum_analysis(
        self, save_path: Path, figsize: Tuple[int, int]
    ):
        ...
        ax1.plot(
            freq_centers, input_spectrum, "b-", label="Input Spectrum", linewidth=2
        )
        ax1.plot(
            freq_centers, output_spectrum, "r-", label="Output Spectrum", linewidth=2
        )
        ax1.set_xlabel("Frequency (normalized)")
        ax1.set_ylabel("Magnitude")
        ax1.set_title(
            f'Frequency Spectrum Comparison - {last_block_key.replace("_fft_input", "")}'
        )
        ax1.legend()
        ...
        ax2.annotate(
            f"Low Freq Ratio: {ratio_spectrum[low_freq_idx]:.2f}",
        ...
        ax2.annotate(
            f"High Freq Ratio: {ratio_spectrum[high_freq_idx]:.2f}",
        ...
        plt.tight_layout()
        plt.savefig(
            save_path / "frequency_spectrum_analysis.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
