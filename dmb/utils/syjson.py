...
class SyncedDict(InnerIterObject):
    ...
    def keys(self):
        return self.root.var(keychain=self.keychain).keys()

    def values(self):
        return self.root.var(keychain=self.keychain).values()

    def items(self):
        return self.root.var(keychain=self.keychain).items()
