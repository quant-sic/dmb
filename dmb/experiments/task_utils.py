"""Contains utilities for training and evaluating models."""
...
def print_config_tree(
    ...
    for field_cfg in cfg:
        if field_cfg not in queue:
            queue.append(field_cfg)

    # generate config tree from queue
    for fiel_queue in queue:
        branch = tree.add(fiel_queue, style=style, guide_style=style)

        config_group = cfg[fiel_queue]
...
def enforce_tags(cfg: DictConfig, save_to_file: bool = False) -> None:
    ...
    if not cfg.get("tags"):
        hydra_conf = HydraConfig().cfg
        if hasattr(hydra_conf, "hydra") and hydra_conf is not None:
