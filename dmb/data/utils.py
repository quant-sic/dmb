...
class SquareSymmetryGroupAugmentations(object):
    ...
    def __call__(self, xy:Union[torch.Tensor,Tuple[torch.Tensor,torch.Tensor]]) -> Union[torch.Tensor,Tuple[torch.Tensor,torch.Tensor]]:
        ...
        # apply symmetry trasnformations to x and y
        ...
        def map_if_not_none(fn:Callable[[torch.Tensor],torch.Tensor],x:Optional[torch.Tensor]) -> Optional[torch.Tensor]:
            if x is None:
                return None
            else:
                return fn(x)
        ...
        if rnd< 1/8: # unity
            ...
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.rot90(x,1,[-2,-1]),xy),(x,y))
        elif rnd< 3/8: # rotate 180 left
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.rot90(x,2,[-2,-1]),xy),(x,y))
        elif rnd< 4/8: # rotate 270 left
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.rot90(x,3,[-2,-1]),xy),(x,y))
        elif rnd<5/8: # flip x
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.flip(x,[-2]),xy),(x,y))
        elif rnd<6/8: # flip y
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.flip(x,[-1]),xy),(x,y))
        elif rnd<7/8: # reflection x=y
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.transpose(x,-2,-1),xy),(x,y))
        else: # reflection x=-y
            x,y = map_if_not_none(lambda xy: map_if_not_none(lambda x: torch.flip(torch.transpose(x,-2,-1),[-2,-1]),xy),(x,y))
...
class TupleWrapper(object):
    ...
    def __call__(self, x:Union[torch.Tensor,Tuple[torch.Tensor,torch.Tensor]]) -> Union[torch.Tensor,Tuple[torch.Tensor,torch.Tensor]]:
