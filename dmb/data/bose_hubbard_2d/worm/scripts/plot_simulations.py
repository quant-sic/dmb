    ...
    for i in tqdm(range(len(ds))):
        for observable in ds[i][1].keys():
            ...
            try:
                ...
                ax[0].imshow(outputs[observable]["mean"]["value"].reshape(
                    int(np.sqrt(len(outputs[observable]["mean"]["value"]))),
                    -1))
                ax[0].set_title(observable)

                ax[1].imshow(outputs[observable]["mean"]["error"].reshape(
                    int(np.sqrt(len(outputs[observable]["mean"]["error"]))),
                    -1))
                ...
                # relative error
                ax[3].imshow(outputs[observable]["mean"]["error"].reshape(
                    int(np.sqrt(len(outputs[observable]["mean"]["error"]))),
                    -1) / outputs[observable]["mean"]["value"].reshape(
                        int(np.sqrt(len(
                            outputs[observable]["mean"]["value"]))), -1))
                ...
                fig.colorbar(
                    ax[1].imshow(outputs[observable]["mean"]["error"].reshape(
                        int(np.sqrt(len(
                            outputs[observable]["mean"]["error"]))), -1)),
                    ax=ax[1],
                )
                fig.colorbar(
                    ax[2].imshow(inputs.reshape(int(np.sqrt(len(inputs))),
                                                -1)),
                    ax=ax[2],
                )
                fig.colorbar(
                    ax[3].imshow(outputs[observable]["mean"]["error"].reshape(
                        int(np.sqrt(len(
                            outputs[observable]["mean"]["error"]))), -1
                    ) / outputs[observable]["mean"]["value"].reshape(
                        int(np.sqrt(len(
                            outputs[observable]["mean"]["value"]))), -1)),
                    ax=ax[3],
                )
