...
def get_simulation_valid(
    simulation_dir: Path, redo: bool = False, log_fn: callable = None
) -> bool:
    try:
...
class BoseHubbardDataset(IdDataset, _PhaseDiagramSamplesMixin):
    ...
    def _clean_sim_dirs(
        ...
        valid_sim_dirs = ProgressParallel(
            total=len(sim_dirs), desc="Filtering valid simulations", use_tqdm=verbose
        )(
        ...
        if max_density_error is not None:
            sim_dirs = list(
                itertools.compress(
                    sim_dirs,
                    ProgressParallel(
                        total=len(sim_dirs), desc="Filtering by error", use_tqdm=verbose
                    )(
