...
import numpy as np
import itertools
from dmb.data.bose_hubbard_2d.network_input import net_input,net_input_dimless_const_parameters
from tqdm.auto import tqdm
from torch.utils.data import DataLoader
import matplotlib.pyplot as plt

def phase_diagram_uniform_inputs_iter(n_samples,zVU=1.0,muU_range=(-0.1,3.1),ztU_range=(0.05,0.85)):
    ...
    for i in range(n_samples*n_samples): 
        yield MUU[i],ZTU[i],net_input_dimless_const_parameters(muU=np.full((16,16),fill_value=MUU[i]),ztU=ZTU[i],zVU=zVU,cb_projection=True)
...
def plot_phase_diagram(model,n_samples=250,zVU=1.0):
    ...
    for name,reduction in reductions.items():
        ...
        plt.pcolormesh(ZTU.view(n_samples,n_samples),MUU.view(n_samples,n_samples),reduction(density).view(n_samples,n_samples))
