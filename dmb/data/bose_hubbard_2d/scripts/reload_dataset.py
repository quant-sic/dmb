...
if __name__ == "__main__":
    ...
    dm = BoseHubbardDataModule(data_dir=REPO_DATA_ROOT/"bose_hubbard_2d",batch_size=64,clean=True,reload=True,verbose=True,observables=["Density_Distribution"])
    dm.setup("fit")
    ...
    with ProgressParallel(n_jobs=10,total=len(dm.dataset)) as parallel:
        data = parallel(delayed(partial(dm.dataset.__getitem__,reload=True))(i) for i in tqdm(range(len(dm.dataset))))
