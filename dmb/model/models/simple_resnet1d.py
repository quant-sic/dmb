...
class ResNet1d(nn.Module):
    ...
    def __init__(self, kernel_size_first, kernel_size_rest, depth, number_of_filters, nr_of_observables, n_base_channels=8, channel_increase_rate=2):
        super().__init__()

        self.depth = depth
        out_channels = n_base_channels

        self.first_block = [conv1d(
            in_channels=1, out_channels=out_channels, kernel_size=kernel_size_first,bias=False),
        ...
        # final prediction
        self.last_block_out = [nn.BatchNorm1d(out_channels), nn.ReLU(), conv1d(
            in_channels=out_channels, out_channels=60, kernel_size=kernel_size_rest,bias=False), nn.BatchNorm1d(60), nn.Tanh(), conv1d(
            in_channels=60, out_channels=nr_of_observables, kernel_size=kernel_size_rest,bias=True)]
